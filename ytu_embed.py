...
import os, time, json, math, torch, numpy as np, sys
...
# Config
# =======================
PARQUET_PATH = None  # will be determined from MILLION_INDEX and available files
# If None, auto-detect among these candidates in order
TEXT_COL = "mdtext"  # e.g., "text_snippet" or "content" or "mdtext"
OUT_EMB_PATH = None  # will be determined from MILLION_INDEX and available files
OUT_META_PATH = None  # will be determined from MILLION_INDEX and available files

MODEL_ID = "ytu-ce-cosmos/turkish-e5-large"
DEVICE = "cuda"
PREFIX = "passage: "  # e5 docs convention; change to "" if undesired
MAX_LEN = 512
BATCH = 512  # starting guess; autotuner will adjust
BATCH_LIMIT = 16384
NUM_WORKERS = 12
PREFETCH = 8
...
# Million-scope selection (hardcoded)
MILLION_INDEX = 1  # 0 => 0m-1m, 1 => 1m-2m, 2 => 2m-3m, ...
START_DOC = MILLION_INDEX * 1_000_000
NUM_DOCS = 1_000_000
...
# Find the parquet file in the embeddings directory structure
if PARQUET_PATH is None:
    # Look for the specific directory pattern: /content/embeddings/docs_Xm_Ym/chunks_Xm_Ym.parquet
    docs_dir = f"docs_{start_label}_{target_end_label}"
    candidate_path = (
        Path(EMBEDDINGS_ROOT)
        / docs_dir
        / f"chunks_{start_label}_{target_end_label}.parquet"
    )

    if candidate_path.exists():
        PARQUET_PATH = str(candidate_path)
    else:
        # Fallback: look for any directory that matches the start pattern
        docs_pattern = f"docs_{start_label}_*"
        matches = sorted(Path(EMBEDDINGS_ROOT).glob(docs_pattern))
        if matches:
            # Use the first matching directory
            docs_dir = matches[0].name
            # Look for chunks file in this directory
            chunks_pattern = f"chunks_{start_label}_*.parquet"
            chunks_matches = sorted(matches[0].glob(chunks_pattern))
            if chunks_matches:
                PARQUET_PATH = str(chunks_matches[0])
            else:
                # If no chunks file found, use the original candidate path
                PARQUET_PATH = str(candidate_path)
        else:
            # If no directory found, use the original candidate path
            PARQUET_PATH = str(candidate_path)

assert Path(PARQUET_PATH).exists(), f"File not found: {PARQUET_PATH}"

# Set output paths based on the parquet file location
parquet_path = Path(PARQUET_PATH)
output_dir = parquet_path.parent  # Same directory as the parquet file
parquet_name = parquet_path.stem  # e.g., "chunks_2m_3m"

# Extract the range from the parquet filename (e.g., "2m_3m" from "chunks_2m_3m")
if parquet_name.startswith("chunks_"):
    range_part = parquet_name[7:]  # Remove "chunks_" prefix
else:
    range_part = parquet_name

# Set output paths
if OUT_EMB_PATH is None:
    OUT_EMB_PATH = str(output_dir / f"embeddings_{range_part}.fp16.npy")
if OUT_META_PATH is None:
    OUT_META_PATH = str(output_dir / f"meta_{range_part}.json")

# Check if embedding file already exists
if Path(OUT_EMB_PATH).exists():
    print(f"Embedding file already exists: {OUT_EMB_PATH}")
    print("Terminating to avoid overwrite.")
    sys.exit(0)
...
print(f"[info] Output embeddings: {OUT_EMB_PATH}")
print(f"[info] Output meta: {OUT_META_PATH}")
